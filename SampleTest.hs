...
hotter number [] = []
hotter number ((place, temperature) : xs)
    | number < temperature = place : hotter number xs
    | otherwise = hotter number xs
...
insert number Null = Node number Null
insert number (Node value rest)
    | number <= value = Node number (Node value rest)
    | otherwise = Node value (insert number rest)
